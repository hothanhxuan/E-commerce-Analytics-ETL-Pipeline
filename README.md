...
> **Note:** This repository is intended for **technical readers** — engineers, data practitioners, and technical leads. If you are looking for a business-focused overview of this project, please visit the [Portfolio Page](https://hothanhxuan.github.io/projects/etl-pipeline.html)  
...
11. [⚠️ Data Audit & Limitations](#data-audit)
12. [📊 Power BI Dashboards](#dashboards)
...
Customers are scored using **NTILE(5)** across three axes, then mapped to **5 actionable macro segments**:
...
| Segment | R Score | F Score | Profile / Action |
|---|---|---|---|
| 👑 VIP / Best Customers | R ≥ 3 | F ≥ 4 | Most valuable, reward and upsell premium products |
| 🌱 Growing / Potential | R ≥ 4 | F ≤ 3 | New or promising, nurture with engagement and offers |
| ⚠️ Needs Attention | R=3/F≤3 or R=2/F=2| Needs re-engagement to prevent slipping |
| 🔴 At Risk | R ≤ 2 | F ≥ 4 | High priority, urgent retention effort needed |
| ❌ Lost / Inactive | Other | Other | Churned, low-cost reactivation attempt |
...
<a id="data-audit"></a>
## ⚠️ Data Audit & Limitations

During the ETL and Data Modeling phase, a comprehensive audit of the raw data revealed structural anomalies in the origin datasets—specifically within the tracking events data. These anomalies mathematically prevent the implementation of certain standard e-commerce analytical models (such as Conversion Funnels and Drop-off Analysis). 

**1. Synthetic & Uniformly Distributed Tracking Data**
* The web tracking dataset (`cart_events.json.gz`) contains synthetic data generated by a randomized script, resulting in a completely uniform distribution. All 14 `event_type` categories (e.g., `view_item`, `add_to_cart`, `purchase`) contain exactly ~215,000 records each. 
* **Impact:** In a real-world scenario, traffic naturally drops off at each stage of the customer journey. Because the volume of `purchase` events exactly equals `view_item` events, attempting to build a **Funnel Chart** or calculate a **Conversion Rate** yields mathematically impossible results (e.g., >100% conversion rates or negative abandonment rates).

**2. Data Contradictions (Ghost Transactions)**
* The system recorded thousands of "purchase" tracking events for `customer_id`s that have **zero actual transactions** in the `fact_orders` database.
* **Impact:** Analyzing Average Touchpoints or Attribution modeling using this data produces illogical insights. We identified that ~70,370 accounts (70% of the customer base) are "ghost" customers with no real purchase history, despite what the tracking log suggests.

**3. Hardcoded Timestamps in Order Data**
* In the Shopify source data, all 500,000 generated orders share an identical, hardcoded hour timestamp of exactly **15:00 (3:00 PM)**. 
* **Impact:** This unnatural spike renders "Peak Purchasing Hour" or intraday behavior analysis skewed and less reflective of genuine consumer behavior.

**4. Overlapping Primary Keys Across Source Systems**
...
**Solution:** To maintain the integrity and accuracy of the dashboards, the analytical strategy was pivoted to focus exclusively on **verified transactional data** (`fact_orders`, `fact_payments`, `dim_customers`). The dashboards now prioritize actionable, reliable insights—such as **RFM Customer Segmentation (5 Macro Segments)**, **Net Cashflow Trends**, and **Payment Gateway Performance**—ensuring the final BI solution delivers true business value despite the origin data limitations.
